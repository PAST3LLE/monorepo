...
import { useIsMediumMediaWidth, useIsSmallMediaWidth } from '@past3lle/hooks'
...
import { StyledHeaderButton } from './common'
...
export function ConnectionInfoButton() {
  ...
  const isMediumWidth = useIsMediumMediaWidth()

  if (isSmallWidth) {
    return (
      <StyledHeaderButton bgImage={logoUrlMaps.SPRAY_ACCOUNT_DDPX_URL_MAP ?? undefined} onClick={onAccountClick}>
    ...
    return (
      <ConnectionInfoContainer
        ...
        width={!isMediumWidth ? '26rem' : '19rem'}
        minWidth={!isMediumWidth ? '24rem' : 'unset'}
        marginTop="0.6rem"
        gap="0.5rem"
      ...
      >
        ...
        >
          <ConnectionStatusWrapper isConnected={!!address}>
            ...
            <small>{`${
              address
                ? truncateAddress(address, { type: !isMediumWidth ? 'long' : 'short' })
                : !isMediumWidth
                ? 'TO VIEW SKILLS'
                : ''
            }`}</small>
